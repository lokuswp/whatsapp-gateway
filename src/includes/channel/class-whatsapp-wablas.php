...
class Notification_Whatsapp_Wablas extends Notification\Gateway {

    protected string $id = 'notification-whatsapp-wablas';
    protected string $name = 'Wablas';
    protected string $type = 'whatsapp';

    protected array $country = [ 'ID', 'US' ];
    protected array $docs_url = array(
        'id_ID' => 'https://lokuswp.id/panduan/lokuswp/notifikasi/integrasi/wablas.html',
        'en_US' => 'https://lokuswp.com/guides/lokuswp/notifications/integration/wablas.html',
    );
    ...
    public function __construct() {
        ...
        // add_action( 'wp_ajax_lokuswp_notification_wablas_test', [ $this, 'action_test' ] );
    }
    ...
    public function prepare_data( $notification_obj ) {
        ...
        return apply_filters( 'lokuswp/notification/whatsapp/wablas/data', $data, $notification_obj->transaction_id );
    }
    ...
    public function templating( $role, $notification_obj ) {
        ...
        $template = apply_filters( 'lokuswp/notification/whatsapp/wablas/templating', $template, $data );
        // $template = str_replace( "{{billing}}", lwp_get_notification_block_billing_text( $locale, $notification_obj->transaction_id ), $template );

        do_action( 'lokuswp/notification/whatsapp/wablas/after_templating', $notification_obj );
    ...
    public function send( array $notification ): bool {
        ...
        try {
            ...
            $request = wp_remote_post( $apiurl . '/api/send-message', array(
                'headers' => array(
...
Notification\Manager::register( new Notification_Whatsapp_Wablas() );