...
class Notification_Whatsapp_StarSender extends Notification\Gateway {
    ...
    protected array $docs_url = array(
        'id_ID' => 'https://lokuswp.id/panduan/lokuswp/notifikasi/integrasi/starsender.html',
        'en_US' => 'https://lokuswp.com/guides/lokuswp/notifications/integration/starsender.html',
    );
    ...
    public function __construct() {
        ...
        // add_action( 'wp_ajax_lokuswp_notification_starsender_test', [ $this, 'action_test' ] );
    }
    ...
    public function prepare_data( $notification_obj ) {
        ...
        return apply_filters( 'lokuswp/notification/whatsapp/starsender/data', $data, $notification_obj->transaction_id );
    }
    ...
    public function templating( $role, $notification_obj ) {
        ...
        $template = apply_filters( 'lokuswp/notification/whatsapp/starsender/templating', $template, $data );
        // $template = str_replace( "{{billing}}", lwp_get_notification_block_billing_text( $locale, $notification_obj->transaction_id ), $template );

        do_action( 'lokuswp/notification/whatsapp/starsender/after_templating', $notification_obj );
    ...
    public function config() {
        if ( file_exists( dirname( __FILE__ ) . '/admin-config/settings-starsender.php' ) ) {
            require_once dirname( __FILE__ ) . '/admin-config/settings-starsender.php';
        }
    }
...
Notification\Manager::register( new Notification_Whatsapp_StarSender() );