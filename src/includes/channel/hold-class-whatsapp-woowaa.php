...
class Notification_Whatsapp_WooWaa extends Notification\Gateway {

    protected string $id = 'notification-whatsapp-woowaa';
    protected string $name = 'WooWaa';
    protected string $type = 'whatsapp';

    protected array $country = [ 'ID', 'US' ];
    protected array $docs_url = array(
        'id_ID' => 'https://lokuswp.id/panduan/lokuswp/notifikasi/integrasi/woowaa.html',
        'en_US' => 'https://lokuswp.com/guides/lokuswp/notifications/integration/woowaa.html',
    );
    ...
    public function __construct() {
        ...
        // add_action( 'wp_ajax_lokuswp_notification_woowaa_test', [ $this, 'action_test' ] );
    }
    ...
    public function prepare_data( $notification_obj ) {
        ...
        return apply_filters( 'lokuswp/notification/whatsapp/woowaa/data', $data, $notification_obj->transaction_id );
    }
    ...
    public function templating( $role, $notification_obj ) {
        ...
        $template = apply_filters( 'lokuswp/notification/whatsapp/woowaa/templating', $template, $data );
        // $template = str_replace( "{{billing}}", lwp_get_notification_block_billing_text( $locale, $notification_obj->transaction_id ), $template );

        do_action( 'lokuswp/notification/whatsapp/woowaa/after_templating', $notification_obj );
    ...
    public function send( array $notification ): bool {
        ...
        try {
            ...
            $request = wp_remote_post('https://woowaa.online/api/sendText?message=' . rawurlencode($message) . '&tujuan=' . rawurlencode($phone . '@s.whatsapp.net'), array(
                'headers' => array(
...
Notification\Manager::register( new Notification_Whatsapp_WooWaa() );