...
//			$data['status']   = lwc_get_order_meta( $trx_id, '_order_status', true );
		...
		return apply_filters( 'lokuswp/notification/whatsapp/' . $this->slug . '/data', $data, $notification_obj->transaction_id );
	}
	...
	public function templating( $role, $notification_obj ) {
		...
		$template = apply_filters( 'lokuswp/notification/whatsapp/' . $this->slug . '/templating', $template, $data );
		// $template = str_replace( "{{billing}}", lwp_get_notification_block_billing_text( $locale, $notification_obj->transaction_id ), $template );

		do_action( 'lokuswp/notification/whatsapp/' . $this->slug . '/after_templating', $notification_obj );
...
//		ray($notification)->green();
	...
	public function config() {
		...
		if ( isset( $_GET['page'] ) && in_array( sanitize_key( $_GET['page'] ), $white_list ) && isset( $_GET['tab'] ) && $_GET['tab'] == "notification" &&
		     file_exists( dirname( __FILE__ ) . '/admin-config/settings-' . $this->slug . '.php' ) ) {
			require_once dirname( __FILE__ ) . '/admin-config/settings-' . $this->slug . '.php';
		}
	}
